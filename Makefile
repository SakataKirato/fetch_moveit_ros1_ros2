...
humble.create:
	@./docker/scripts/run.bash --ros-distro=humble $(if $(volumes),--volumes=$(volumes)) $(if $(name),--name=$(name))

# Create containers with CUDA support
humble.create.cuda:
	@./docker/scripts/run.bash --ros-distro=humble --use-cuda $(if $(volumes),--volumes=$(volumes)) $(if $(name),--name=$(name))

humble.create.jetson.35.2.1:
	@./docker/scripts/run.bash --ros-distro=humble --l4t=35.2.1 $(if $(volumes),--volumes=$(volumes)) $(if $(name),--name=$(name))
